  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100 w-full">
      ...
      <aside 
        ...
        md:translate-x-0 transform fixed md:static inset-y-0 left-0 z-30 w-72 bg-white 
        border-r border-blue-100 shadow-xl md:shadow-none transition-transform duration-300 ease-in-out`}
      ...
      >
        <div className="flex flex-col h-full px-5 py-8 overflow-y-auto">
          ...
          <div className="flex flex-col items-center mb-8 px-4 text-center">
            ...
            <span className="text-sm text-blue-500 bg-blue-50 px-3 py-1 rounded-full mt-1">
              {usuario.rol}
            </span>
          </div>
          ...
          <nav className="flex flex-col gap-2 w-full mb-auto">
            {MENU_PACIENTE.map(({ label, icon: Icon, path }) => (
              <button
                key={path}
